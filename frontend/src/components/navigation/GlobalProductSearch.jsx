...
function GlobalProductSearch({ mobile = false, onNavigate, desktopWide = false, industrial = false }) {
  ...
  return (
    ...
    >
      <label className="relative block">
        ...
        <FaSearch
          className={[
            'pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-xs',
            industrial ? 'text-white/70' : 'text-brand-muted',
          ].join(' ')}
        />
        <input
          type="search"
          ...
          className={[
            industrial
              ? 'w-full border border-white/12 bg-[linear-gradient(180deg,rgba(105,110,118,0.96),rgba(64,68,75,0.96))] pl-9 pr-3.5 text-sm text-white outline-none transition placeholder:text-white/58 focus:border-brand-green focus:bg-[linear-gradient(180deg,rgba(116,121,130,0.98),rgba(72,76,84,0.98))]'
      ...
      {showDropdown ? (
        <div
          className={[
            'absolute left-0 right-0 top-[calc(100%+0.6rem)] z-[60] overflow-hidden rounded-[1.35rem] border shadow-[0_26px_70px_rgba(35,33,32,0.14)]',
            industrial
              ? 'border-white/10 bg-[linear-gradient(180deg,rgba(66,71,79,0.98),rgba(35,38,44,0.99))]'
        ...
        >
          {suggestions.length ? (
            <div className="max-h-[24rem] overflow-y-auto py-2">
                ...
                if (suggestion.type === 'product') {
                  return (
                    <button
                      ...
                      className={`flex w-full items-start gap-3 px-4 py-3 text-left transition ${industrial ? 'hover:bg-white/8' : 'hover:bg-brand-surface'}`}
                    >
                      <div className="mt-1 h-2.5 w-2.5 rounded-full bg-brand-green" />
                      <div className="min-w-0">
                        <p className={`truncate text-sm font-semibold ${industrial ? 'text-white' : 'text-brand-ink'}`}>
                          {suggestion.product.name}
                        </p>
                        <p className="mt-1 text-xs uppercase tracking-[0.16em] text-brand-green">
                          {suggestion.product.subcategory}
                        </p>
                        <p className={`mt-1 truncate text-xs ${industrial ? 'text-white/64' : 'text-brand-muted'}`}>
              ...
              <div className={`border-t px-4 py-3 ${industrial ? 'border-white/10' : 'border-brand-border'}`}>
                <button
                  type="button"
            ...
            <div className="px-4 py-4">
              <p className={`text-sm ${industrial ? 'text-white/68' : 'text-brand-muted'}`}>
